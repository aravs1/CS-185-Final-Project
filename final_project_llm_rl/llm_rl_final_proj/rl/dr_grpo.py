    ...
    def update(
        ...
        for _ in range(cfg.ppo_epochs):
            for mb in iter_minibatches(
                ...
                advantage_tokens = adv.unsqueeze(1)

                unclipped = ratio * advantage_tokens
                clipped = torch.clamp(ratio, 1.0 - cfg.clip_eps, 1.0 + cfg.clip_eps) * advantage_tokens
                ...
                clipfrac = masked_mean(clipped_mask.float(), mask)
