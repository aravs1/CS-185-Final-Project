    ...
    def update(
        ...
        for _ in range(cfg.ppo_epochs):
            for mb in iter_minibatches(
                ...
                token_log_ratio = torch.clamp(new_logp - mb.old_logprobs, min=-20, max=20)
                seq_log_ratio = masked_mean_per_row(token_log_ratio, mask)
                
                seq_ratio = torch.exp(seq_log_ratio)

                unclipped = seq_ratio * adv
                clipped = torch.clamp(seq_ratio, 1.0 - cfg.clip_eps, 1.0 + cfg.clip_eps) * adv
                seq_obj = torch.min(unclipped, clipped)
                ...
                clipped_mask = (seq_ratio < (1.0 - cfg.clip_eps)) | (seq_ratio > (1.0 + cfg.clip_eps))
                clipfrac = clipped_mask.float().mean()
