...
def compute_offline_preference_loss(
    *,
    ...
    if algo == "dpo":
        ...
        #polciy_margin_sum = (policy_margin_sum - ref_margin_sum) 
        ...
        logits = (policy_margin_sum - ref_margin_sum) * beta
        # TODO(student): replace this with the DPO logistic loss.
        losses = F.logsigmoid(logits).mul(-1.0)
        metrics.update(
            {
        ...
        chosen_rewards_sorted = torch.sort(chosen_rewards)[0]
        rejected_rewards_sorted = torch.sort(rejected_rewards)[0]

        quantile_gap = (chosen_rewards_sorted - rejected_rewards_sorted) * beta
